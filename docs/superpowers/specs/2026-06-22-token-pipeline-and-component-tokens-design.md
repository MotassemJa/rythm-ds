...
`stencil.config.ts` adds `globalStyle: '../tokens/dist/tokens.css'`. Stencil bundles this through its own CSS pipeline, and the component loader auto-injects it into the consuming document's `<head>` — so loading `@rythm-ds/core` wires up tokens automatically, rather than requiring consumers (including the dev playground) to manually `<link>` the file. The `copy` entry in the `www` output target's config (which exists solely to get `tokens.css` next to the playground's `index.html`) is removed since it's now redundant, and the manual `<link rel="stylesheet" href="/tokens.css" />` in `index.html` is removed too.
